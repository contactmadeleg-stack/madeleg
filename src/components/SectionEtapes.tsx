    ...
    <section id="comment-ca-marche" className="mx-auto max-w-6xl px-4 py-16 sm:py-24 scroll-mt-20">
      ...
      <Reveal stagger>
        <div className="relative grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div
            aria-hidden
            className="hidden lg:block absolute top-[2.75rem] left-[12.5%] right-[12.5%] border-t-2 border-dashed -z-10"
            style={{ borderColor: "var(--border-subtle)" }}
          />
          {ETAPES.map(({ numero, icone: Icone, titre, texte }) => (
            <RevealItem
              key={numero}
              className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-card)] p-6 relative transition-all duration-200 hover:-translate-y-1.5 hover:shadow-xl"
            >
              <span className="font-titres absolute top-4 right-5 text-3xl font-extrabold" style={{ color: "var(--border-subtle)" }}>
                {numero}
              </span>
              <span className="flex items-center justify-center w-11 h-11 rounded-full bg-gradient-to-br mb-4 shadow-sm" style={{ backgroundImage: "linear-gradient(to right, var(--emerald-50), var(--amber-100))", color: "var(--emerald-600)" }}>
