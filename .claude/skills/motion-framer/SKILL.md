...
### Notes on this local copy

Only this SKILL.md reference doc was provided — the upstream repo's bundled `references/`, `scripts/`, and `assets/starter_motion/` (Python generators, starter template) were not included and are not present here. This file is documentation only; it does not install the `framer-motion`/`motion` npm package. Run `npm install motion` in the project when animations are actually implemented.
